...
class ImageControllerTest extends TestCase
{
    ...
    {
        return [
            'Auth middleware is not applied to account.profile-image.edit route' => ['auth', 'account.profile-image.edit'],
            'Auth middleware is not applied to account.profile-image.update route' => ['auth', 'account.profile-image.update'],
            'Verified middleware is not applied to account.profile-image.edit route' => ['verified', 'account.profile-image.edit'],
            'Verified middleware is not applied to account.profile-image.update route' => ['verified', 'account.profile-image.update'],
        ];
    }
    ...
    public function profile_image_edit_view_can_be_rendered()
    {
        $response = $this->actingAs(create_user())
            ->get(route('account.profile-image.edit'));

        $response->assertStatus(Response::HTTP_OK)
            ->assertViewHasForm('id="update_profile_image"', 'patch', route('account.profile-image.update'))
            ->assertFormHasCSRF()
    ...
    public function form_is_inside_turbo_frame()
    {
        ...
        $response->assertStatus(Response::HTTP_OK)
            ->assertElementHasChild(
                ...
                'form[action="'.route('account.profile-image.update').'"]'
            );
    }
    ...
    public function user_can_update_profile_image()
    {
        ...
        $response = $this->actingAs($user)
            ->from(route('account.profile-image.edit'))
            ->patch(route('account.profile-image.update'), [
    ...
    {
        $this->assertRouteUsesFormRequest('account.profile-image.update', UpdateImageRequest::class);
    }
}
